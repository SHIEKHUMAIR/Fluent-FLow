  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const unit = unitsResult.data.find(u => u.unit_number === 3);
        if (!unit) {
          setError("Unit 3 not found");
          return;
        }

        // Get lessons for unit 3
        const lessonsResult = await apiGet(API_ENDPOINTS.LESSONS.BY_UNIT(unit.id));
                ...
                <div className="p-8">
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <span className={`text-sm font-semibold px-3 py-1.5 rounded-full ${lesson.category === 'beginner' ? 'bg-emerald-100 text-emerald-700' :
                      ...
                      {lesson.category ? lesson.category.charAt(0).toUpperCase() + lesson.category.slice(1) : 'Beginner'}
                    </span>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-3">
                      <div className="w-full bg-slate-200 rounded-full h-3 max-w-[120px]">
                        <div
                          className={`h-3 rounded-full ${progress === 100 ? 'bg-emerald-500' : 'bg-blue-900'}`}
                  ...
                  {/* Button */}
                  <Link href={lesson.path || `/modules/unit03/lesson${lesson.lesson_number}`}>
                    <button
